...
Packager.prototype.init = function(options, fn) {
    ...
    io.mkdir( output , function(){

    	io.write(path.join(output, 'nspackage.json'), JSON.stringify(po));
	    io.write(path.join(output, '.nsignore'), nsignore);
...
Packager.prototype.install = function(options, fn) {
	...
	var tgz = options.name
	  , output = path.join(options.output, 'nitrus/packages/');
 ...
 * @param {Function} fn
 */
Packager.prototype.pack = function(options, fn) {
	var self = this;

	var input = options.pack || '.';

	var nsp =  JSON.parse(fs.readFileSync(path.join(input, 'nspackage.json')));
	var otgz = path.join(input, nsp.name + '.tgz');

	var compress = new targz().compress(otgz, function(err){
	    if(err)
	        console.log(err);
